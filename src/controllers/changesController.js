const { getChanges, getReviews, getUserData, getUserStats, getCrStats, getOpenCrs, getGraphStats, getPending } = require('../utils/changesUtil');
...
const getCrStatistics = async (req, res) => {
  const userId = req.params.extId;
  const startDate = req.query.startDate;
  const endDate = req.query.endDate;
  if(!userId) return res.status(400).send({ message: 'User Id is required' });
  try {
    const changes = await getCrStats(userId, startDate, endDate);
    if(!changes) return res.status(404).send({ message: 'Changes not found' });
    res.status(200).send(changes);
  } catch (error) {
    console.log(error);
...
const getReviewsByFilter = async (req, res) => {
  const userId = req.params.extId;
  const startDate = req.query.startDate;
  const endDate = req.query.endDate;
  if(!userId) return res.status(400).send({ message: 'User Id is required' });
  try {
    const changes = await getGraphStats(userId, startDate, endDate);
    if(!changes) return res.status(404).send({ message: 'Changes not found' });
    res.status(200).send(changes);
  } catch (error) {
    console.log(error);
...
const getPendingReviews = async (req, res) => {
  ...
  if(!userId) return res.status(400).send({ message: 'User Id is required' });
  try {
    const changes = await getPending(userId, startDate, endDate);
    if(!changes) return res.status(404).send({ message: 'Changes not found' });
    res.status(200).send(changes);
  } catch (error) {
    console.log(error);
...
module.exports = { getChangesByOwner, getChangesByReviewer, getUserChanges, getUserStatistics, getCrStatistics, getOpenChanges, getReviewsByFilter, getPendingReviews }